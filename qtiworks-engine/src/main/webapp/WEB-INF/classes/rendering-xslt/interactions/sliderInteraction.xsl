...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="qti:sliderInteraction">
    ...
    <div class="{local-name()}">
      ...
      <xsl:variable name="value" select="jqti:get-response-value(@responseIdentifier)"/>
      <xsl:variable name="is-discrete" select="jqti:get-response-declaration(/, @responseIdentifier)/@baseType='integer'" as="xs:boolean"/>
      ...
      <div class="sliderInteraction">
        <xsl:choose>
          <xsl:when test="$orientation='horizontal'">
            <div class="sliderHorizontal">
              <div class="sliderWidget" id="jqtiid_slider_{@responseIdentifier}"></div>
              <div class="sliderValue"><span id="jqtiid_slidervalue_{@responseIdentifier}"><xsl:value-of select="$value"/></span></div>
            </div>
          </xsl:when>
          <xsl:otherwise>
        ...
        <input type="hidden" name="jqtiresponse_{@responseIdentifier}" value="{$value}"/>
        <script type="text/javascript">
          $(document).ready(function() {
            JQTIItemRendering.registerSliderInteraction('<xsl:value-of
